    ...
    public function listcom(CommandeRepository $commandeRepository): Response
{
     ...
     */
    public function addToPanier(Produit $produit, SessionInterface $session)
    {
    ...
    public function displayPanier(SessionInterface $session, ProduitRepository $repo)
    {
        ...
        $total = 0;
        $s = 0;
        foreach ($panier as $id => $nbr_prods) {
            $produit = $repo->find($id);
            $dataPanier[] = [
                "produit" => $produit,
                "nbr_prods" => $nbr_prods
            ];
            $total += $produit->getPrix() * $nbr_prods;
        }
    ...
    /**
     * @Route("/removepanier/{id}", name="remove_panier", requirements={"id":"\d+"})
     */
    public function removeFromPanier($id, SessionInterface $session)
    {
        $panier = $session->get('panier', []);
